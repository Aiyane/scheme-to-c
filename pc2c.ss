...
(module (pc2c
  ...
  (define check-args
    (lambda (union args)
      ...
      [(memq (car args) (cdr args))
        (errorf 'define-union "duplicated variant `~a' in union `~a'\n" (car args) (car union))]
  ...
  (define add-union
    (lambda (union)
      (if (not (lookup-union (car union)))
          ...
          (errorf 'define-union "duplicated definition of union-type `~a'\n" (car union)))))
  ...
  (define pc2c
    (lambda (file-name source-name header-name)
      ...
      (init-storage)
      (let* ([source-code (file->list file-name)]
             [lexed-code (run (car source-code))]
             [decl* (append (file->list "interp.ss") (list lexed-code))])
      ...
      (apply string-append
            (map (lambda (elt)
                    (cond
                      ...
                      [(string? elt) elt]
                      [else (errorf 'pc2c-append "Invalid argument ~s" elt)]))
                  args))))
  ...
  (define apply-env
    (lambda (env x)
      (pmatch env
          [(empty-env) (errorf 'empty-env "unbound variable: ~s" x)]
          [(extend-env ,x^ ,a ,env)
      ...
      (if tail
          (lambda (expr)
            (pmatch expr
              ...
              [(set! ,var ,var1) (guard (eq? var var1)) ""]
              [(set! ,var ,val)
              ...
              (let ((template* (map car c*))
                ...
                (if (not (check-union-case expr name
                                            ...
                                            (map car template*)))
                    (errorf 'union-case "union-case doesn't match definition: `~a'\n"
                            name)
                    (let ([sname (safe name)]
                          [val (safe val)])
                      (pc2c-append
        ...
        ;; [(error ,name ,msg) 
              ...
              [,x (guard (symbol? x)) (safe (apply-env env x))]
              [,x (guard (integer? x)) (pc2c-append "(void *)" x)]
              [(zero? ,x) 
              ...
              (let ((a ((parse-function-body #f env) a)))
                (pc2c-append "(void *)((int)" a " + 1)"))]
              ;; 在 C 中先使用 srand (time (NULL)); 设置随机数种子再调用 rand()
              [(random ,x) 
              (let ((x ((parse-function-body #f env) x)))
  ...
  (define pc2c-source
    (lambda (header-name)
      (let* ([s1 (apply string-append (map pc2c-gen-unions reg-unions))]
        ...
        ;; 设置跳转 _trstr 类型在头文件定义，包括 jmpbuf 与 value 值
        ;; _status 初始化为 0 会无限循环，在 longjmp 后为 1 则不会进入循环
        (let ([s3 (pc2c-append
                  "int mount_tram ()\n"
                  "{\n"
                  "srand (time (NULL));\n"
      ...
      (pmatch decl
        ...
        (if (null? reg*)
            ...
            (string-append
              ...
              (join (map safe reg*) ", *")
              ";\n\n"))]
        ;; 定义 pc 为无参的函数指针，并将变量名赋值给 reg-pc
          ...
          (let ([name (safe name)])
            (let ([enum-tags
              ...
              (pc2c-append
                ...
                "};\n\n"
                structors "\n"))))]
        ;; 定义无参函数，除 main 函数外都是 void，main 函数是 int
    ...
    (let ([pc-file (string-append base-name ".ss")]
      ...
      (let ([compile-command (string-append "gcc -o " base-name " " c-file)]
        ...
        (let ([compile-val (system compile-command)])
          (unless (eqv? compile-val 0)
            (errorf 'compile/run "Compilation command '~s' returned ~s"
